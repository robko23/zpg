...
class TransformationRotate : public Transformation {
  ...
  public:
    explicit TransformationRotate(const glm::vec3 &axis, const float angle)
        : axis(axis), angleRadians(angle) {}

    glm::mat4 apply(glm::mat4 source) override {
        return glm::rotate(source, angleRadians, axis);
    }

    [[nodiscard]] inline float getAngleRadians() const { return angleRadians; }

    inline void setAngleRadians(float rad) { angleRadians = rad; }
};
...
class TransformationScale : public Transformation {
  ...
  public:
    ...
    void setScale(float val) { scale = glm::vec3(val); }
};
...
class TransformationTranslate : public Transformation {
  ...
  public:
    explicit TransformationTranslate(const glm::vec3 &position)
        : position(position) {}
...
class TransformationBuilder {
  ...
  public:
    TransformationBuilder(TransformationBuilder &&other)
    ...
    operator=(const TransformationBuilder &other) = delete;
    ...
    TransformationBuilder scale(float scale) {
        transformations.emplace_back(
            std::make_shared<TransformationScale>(glm::vec3(scale)));
        return *this;
    }

    TransformationBuilder scale(float x, float y, float z) {
        transformations.emplace_back(
            std::make_shared<TransformationScale>(glm::vec3(x, y, z)));
        return *this;
    }

    TransformationBuilder rotateX(float angleRadians) {
        transformations.emplace_back(std::make_shared<TransformationRotate>(
            glm::vec3(1, 0, 0), angleRadians));
        return *this;
    }

    TransformationBuilder rotateY(float angleRadians) {
        transformations.emplace_back(std::make_shared<TransformationRotate>(
            glm::vec3(0, 1, 0), angleRadians));
        return *this;
    }

    TransformationBuilder rotateZ(float angleRadians) {
        transformations.emplace_back(std::make_shared<TransformationRotate>(
            glm::vec3(0, 0, 1), angleRadians));
        return *this;
    }
    ...
    [[nodiscard]] Transformation *at(size_t idx) const {
        DEBUG_ASSERTF(idx <= this->transformations.size(),
                      "Trying to get element outside bouds");
    ...
    replace(size_t idx, std::unique_ptr<Transformation> newTrans) {
        DEBUG_ASSERTF(idx <= this->transformations.size(),
                      "Trying to get element outside bouds");
