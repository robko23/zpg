...
struct ProjectionMatrix {
    ...
    inline static ProjectionMatrix _default() {
        return ProjectionMatrix{.projectionMatrix = glm::mat4(1)};
    }
};

class PerspectiveProjection : public Observer<WindowSize>,
                              public Observable<ProjectionMatrix> {
  private:
    float fov = 60;
    int screenWidth = 0;
    int screenHeight = 0;
    ...
    inline void assertSetSize() const {
        DEBUG_ASSERTF(screenWidth != 0 && screenHeight != 0,
                      "Window dimensions are not set. Did you forget to "
                      "register windowResize callback?")
    }
    ...
    void recalculate() {
        ...
        auto projectionRaw = glm::perspective(glm::radians(fov), aspectRatio,
  ...
  public:
    PerspectiveProjection()
        : Observable<ProjectionMatrix>(ProjectionMatrix::_default()),
          projectionMatrix(ProjectionMatrix::_default()) {}
